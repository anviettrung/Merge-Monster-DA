...
//, bool hasMouse
        ...
        internal void LoadTerrainData(TerrainData terrain)
        {
            ...
            foreach (var treePrototype in arr2)
            {
                var aPath = AssetDatabase.GetAssetPath(treePrototype.prefab);
                var refGUID = AssetDatabase.AssetPathToGUID(aPath);
                AddUseGUID(refGUID);
            }

            var arr3 = terrain.terrainLayers;
            foreach (var terrainLayer in arr3)
            {
                var aPath = AssetDatabase.GetAssetPath(terrainLayer.diffuseTexture);
                var refGUID = AssetDatabase.AssetPathToGUID(aPath);
                AddUseGUID(refGUID);

                var bPath = AssetDatabase.GetAssetPath(terrainLayer.normalMapTexture);
...
//#endif
            ...
            if (type == FR2_AssetType.TERRAIN)
            {
                ...
                if (fromObj is Texture2D)
                {
                    ...
                    {
                        if (arr3[i].diffuseTexture == (Texture2D) fromObj)
                        {
                            found++;
                            arr3[i].diffuseTexture = (Texture2D) toObj;
                        }

                        if (arr3[i].normalMapTexture == (Texture2D) fromObj)
                        {
                            found++;
                            arr3[i].normalMapTexture = (Texture2D) toObj;
                        }
                    }
